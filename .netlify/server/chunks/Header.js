...
const Header = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `<header class="${"svelte-zwqxtg"}">${titleLink ? `<a${add_attribute("href", titleLink, 0)} class="${"section-title svelte-zwqxtg"}">${escape(title)}</a>` : `<div class="${"section-title svelte-zwqxtg"}">${escape(title)}</div>`}
  <a class="${"back"}"${add_attribute("href", link, 0)}>${validate_component(BackButton, "BackButton").$$render($$result, {}, {}, {})}</a>
</header>`;
});
export {
  Header as H
